...
For key, path in argarr{
    ...
    inarr["文本"] := "老黄牛小工具"
    inarr["语音路径"] := "D:\\老黄牛小工具\\ExcelQuery\\temp\\合成语音.mp3"
    inarr[""] := ""
    outarr["语音时长"] := "待返回"
    outarr[""] := ""
    outarr[""] := ""
    ...
    fkeynew := "语音路径"   ;要接收的新文件的标题行名称
    inarr["TencentSecretId"] := arrupdatefromconfigini(inarr["TencentSecretId"], "ApiKeys", "TencentSecretId") 
    inarr["TencentsecretKey"] := arrupdatefromconfigini(inarr["TencentsecretKey"], "ApiKeys", "TencentsecretKey") 
    if (fkeyold!=""){
     inarr[fkeyold] := path
    }
