...
For key, path in argarr{
    ...
    inarr["xfapikey"] := ""
    inarr["文本"] := "老黄牛小工具"
    inarr["生成语音地址"] := "D:\\老黄牛小工具\\ExcelQuery\\temp\\语音.mp3"
    outarr[""] := ""
    outarr[""] := ""
    outarr[""] := ""
    outarr[""] := ""
    ...
    fkeynew := "生成语音地址"   ;要接收的新文件的标题行名称
    inarr["xfappid"] := arrupdatefromconfigini(inarr["xfappid"], "ApiKeys", "xfappid") 
    inarr["xfapisecret"] := arrupdatefromconfigini(inarr["xfapisecret"], "ApiKeys", "xfapisecret") 
    if (fkeyold!=""){
     inarr[fkeyold] := path
    }
