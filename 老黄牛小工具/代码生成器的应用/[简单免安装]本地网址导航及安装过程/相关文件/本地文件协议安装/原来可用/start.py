...
if len(sys.argv) >=2:
   ...
   inputstr="cmd://notepad.exe"
   #inputstr="cmd://2021%E5%B9%B4%E5%BA%A6%E7%A7%91%E7%A0%94%E6%88%90%E6%9E%9C%E7%99%BB%E8%AE%B0%E5%B7%A5%E4%BD%9C"
   print("默认输入为cmd://notepad.exe 方便调试。\n")
...
print(realfolderpath)
print("%lilyfolder%")
if realfolderpath != None:
...
if len(extension) < 1:
   ...
   #time.sleep(3)
   #这里处理文件夹，
   if os.environ.get('LILYFOLDER') != None:
      if inputfile.find("\\") < 0:
            path = os.environ.get('LILYFOLDER')
            if os.path.exists(path + "\\分类工作"):#这里强制改变分类目录提高效率
                path=path + "\\分类工作"
            #print("inputfile1: "+ inputfile)
            #print("path:"+ path)
            #time.sleep(7)
