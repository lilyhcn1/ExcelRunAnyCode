...
def check_remote_desktop_sessions():
    try:
        # 执行 'query session' 命令
        result = subprocess.check_output('query session', stderr=subprocess.STDOUT, text=True, shell=True)
    except subprocess.CalledProcessError as e:
        print("必然出错。", e.output)
        result=e.output

    checkstr="rdp-tcp#"
    if checkstr in result :
        return True
    else:
        return False
...
def main(fd2={}):
    ...
    try:  # 运行函数,最后要生成arr2ret及f64
        ...
        arr2ret["stat"] =check_remote_desktop_sessions()
