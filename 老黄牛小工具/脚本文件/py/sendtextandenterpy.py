...
#按键或快捷键
def press_cv_enter_str(text,sleeptime=2):
    time.sleep(1)
    paste_text(text)
...
#按键或快捷键
def press_cv_enter_arr(textarr,sleeptime=2):
    time.sleep(2)
    for ss in textarr:
        press_cv_enter_str(ss,sleeptime)
...
def main(fd2={}):
    ...
    try:  # 运行函数,最后要生成arr2ret及f64
        ...
        press_cv_enter_arr(split_string_based_on_delimiter(valarr["text"]))
