...
def get_website_title(url):
    try:
        status_code, response = send_request(url)
        if status_code <500: 
            # 获取网页内容的文本
...
def send_request(url, timeout=2, proxy=None):
    buffer = BytesIO()
    c = pycurl.Curl()
    # 设置 URL
    c.setopt(c.URL, url)
    ...
    c.setopt(c.HTTPHEADER, chrome_headers)
    ...
    decompressed_body = zlib.decompress(body, 16+zlib.MAX_WBITS)
    
    return status_code, decompressed_body.decode('utf-8')
