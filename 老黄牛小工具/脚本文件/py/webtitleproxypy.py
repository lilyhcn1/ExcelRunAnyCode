...
def fetch_url_via_proxy(url, proxy_url, timeout=3):
    # 解析代理URL
    parsed_proxy = urlparse(proxy_url)
    proxy_address = parsed_proxy.hostname
    ...
    try:
        ...
        if scheme == 'socks5':
            ...
            raise ValueError("Unsupported proxy scheme. Only 'socks5' and 'http' are supported.")
...
def fetch_url_via_proxy_stat(url, proxy_url, timeout=5):
    # 解析代理URL
    response, status_code = fetch_url_via_proxy(url, proxy_url,timeout)
...
#输入文本
inarr["url"]="http://www.google.com"
inarr["proxy"]="socks5://192.168.7.141:1080"
inarr[""]=""
inarr[""]=""
inarr[""]=""
outarr["state"] = "待返回"
...
def main(fd2={}):
    ...
    try:  # 运行函数,最后要生成arr2ret及f64
        ...
        arr2ret["stat"]=fetch_url_via_proxy_stat(valarr["url"], valarr["proxy"])
