...
inarr["附件地址"]="D:\\老黄牛小工具\\word模板\\二维码.png"
inarr["收件人"]="53053067@qq.com"
inarr["发件人"]=""
inarr["密码"]=""
outarr["发送结果"] = "待返回"
outarr[""] = ""
...
def main(fd2={}):
    ...
    #inarr 主题 附件地址 收件人 发件人 密码
    #outarr 发送结果    
    #fkeyold 附件地址 fkeynew  
    try:  # 运行函数,最后要生成arr2ret及f64
        ...
        password = valarr["密码"]
        sender = username
        receivers = valarr["收件人"]
            # 如名字所示： Multipart就是多个部分
        #print(receivers)
        msg = MIMEMultipart()
        msg['Subject'] = valarr["主题"]
        msg['From'] = sender
        msg['To'] = receivers

        # 下面是文字部分，也就是纯文本
        puretext = MIMEText("正文部分.")
        msg.attach(puretext)

        # 下面是附件部分 ，这里分为了好几个类型

        # 首先是xlsx类型的附件
        xlsxpart = MIMEApplication(open(old_filepath, 'rb').read())
        xlsxpart.add_header('Content-Disposition', 'attachment', filename=old_filepath)
        msg.attach(xlsxpart)
        #print(old_filepath)
        # jpg类型的附件
