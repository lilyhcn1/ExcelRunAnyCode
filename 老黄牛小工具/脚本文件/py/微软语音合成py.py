...
# 教程https://zhuanlan.zhihu.com/p/619612276
async def text2mp3(tt,outputpath="temp.mp3"):
    voice = 'zh-CN-XiaoxiaoNeural'
    rate = '-0%'
    volume = '+0%'
    tts = edge_tts.Communicate(text = tt,voice = voice,rate = rate,volume=volume)
...
#def playtts2(text):#会出错
#    mp3path="temp.mp3"
#    mp3path=asyncio.run(text2mp3(text,mp3path))
#    
#    playsound(mp3path)
...
def main(fd2={}):
    ...
    try:  # 运行函数,最后要生成arr2ret及f64
        ...
        asyncio.run(text2mp3(valarr["文本"],new_filepath))
