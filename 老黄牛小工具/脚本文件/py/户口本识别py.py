...
#输入文本
inarr["TencentSecretId"]=""
inarr["TencentsecretKey"]=""
inarr["图片地址"]="D:\\老黄牛小工具\\word模板\\户口本.jpg"
inarr[""]=""
inarr[""]=""
outarr["HouseholdNumber"] = "待返回"
outarr["HouseholderName"] = "待返回"
outarr["Household"] = "待返回"
outarr["Address"] = "待返回"
outarr["识别返回键值"] = "待返回"
#文件标志
...
def main(fd2={}):
    ...
    #inarr TencentSecretId TencentsecretKey 图片地址  
    #outarr HouseholdNumber HouseholderName Household Address 识别返回键值
    #fkeyold 图片地址 fkeynew  
    try:  # 运行函数,最后要生成arr2ret及f64


        SecretId = valarr["TencentSecretId"]
        SecretKey = valarr["TencentsecretKey"]
        ...
        req = models.ResidenceBookletOCRRequest()   ########修改这里#############
        params = {
            "ImageBase64":f64
        }
        ...
        arr = json.loads(resp.to_json_string())
        #arr = arr["VatRollInvoiceInfos"]
        #print(arr)
        keystr=""
        #for valarr in arr:
        #    key = valarr["Name"]
        ...
        for key, val in arr.items():
            arr2ret[key] = lilyfun.list2str(val,",")
            keystr +=key+","
        
        arr2ret["识别返回键值"] = keystr
