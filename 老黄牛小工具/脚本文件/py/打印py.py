...
def get_prog_id(file_extension):
    return "Word.Application"
    pptstr = ["ppt", "pptx"]
    excelstr = ["xls", "xlsx", "xlsm"]
    jpgstr = ["jpg", "jpeg","png","bmp","gif"]

    if file_extension in pptstr:
        return "PowerPoint.Application"
    elif file_extension in excelstr:
        return "Excel.Application"

    elif file_extension in jpgstr:
...
def print_file(file_path):
    ...
    # 根据文件后缀获取ProgID
    prog_id = get_prog_id(file_extension)
    print(prog_id)
    try:
        # 创建应用程序对象
        app = win32com.client.Dispatch(prog_id)
        app.Visible = False  # 可选：设置应用程序不可见
...
#输入文本
inarr["文件"]="D:\\老黄牛小工具\\word模板\\座位牌2字.docx"
inarr[""]=""
inarr[""]=""
inarr[""]=""
inarr[""]=""
